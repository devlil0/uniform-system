...
export default function ClientesPage() {
  ...
  async function handleSubmit(event) {
    ...
    setSuccess('');
    try {
      await api.createCliente({ nome: form.nome, contato: form.contato });
      setForm(initialForm);
  ...
  const form_aside = (
    <Card variant="secondary" p="20px">
      <SectionHeader title="Novo cliente" description="Preencha os dados para cadastrar um novo cliente." />
      <form onSubmit={handleSubmit}>
        <Stack spacing="14px">
          ...
          <Button type="submit" colorScheme="brand" isLoading={submitting}>Salvar cliente</Button>
        </Stack>
      </form>
    </Card>
  );
  ...
  return (
    <Box pt={{ base: '130px', md: '80px', xl: '80px' }}>
      <Stack spacing="24px">
        ...
        <DataCard title="Clientes" description="Cadastro e listagem de clientes do sistema." aside={form_aside}>
          <TableContainer>
            {loading ? (
              ...
              <Table variant="simple">
                <Thead>
                  <Tr><Th>ID</Th><Th>Nome</Th><Th>Contato</Th><Th /></Tr>
                </Thead>
                <Tbody>
                  {clientes.length ? (
                    clientes.map((c) => (
                    ...
                    <Tr><Td colSpan={4} py="32px" textAlign="center"><Text color={muted}>Nenhum cliente cadastrado.</Text></Td></Tr>
                  )}
                </Tbody>
              </Table>
            )}
